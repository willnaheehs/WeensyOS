...
x86_64_pagetable* process_pagetable;
...
int virtual_memory_map(x86_64_pagetable* pagetable, uintptr_t va,
    ...
    if (perm & PTE_P) {
        ...
        // debug
        if (pa + sz > MEMSIZE_PHYSICAL) {
        console_printf(CPOS(22, 0), 0xC000,
                       "virtual_memory_map: pa=%p sz=%zu MEMSIZE_PHYSICAL=%p\n",
                       pa, sz, (uintptr_t) MEMSIZE_PHYSICAL);
    }
        assert(pa + sz <= MEMSIZE_PHYSICAL); // physical addresses exist
    }
    if(!(perm >= 0 && perm < 0x1000)) {
        log_printf("va: %X\n, pa: %d\n, perms:%d\n", va, pa, perm);}
    ...
    for (; sz != 0; va += PAGESIZE, pa += PAGESIZE, sz -= PAGESIZE) {
        ...
        if ((perm & PTE_P) && l1pagetable) { // if page is marked present
            ...
            int l1_index = PAGEINDEX(va, 3); // level 3 corresponds to L1 page table
            l1pagetable->entry[l1_index] = pa | perm;

        } else if (l1pagetable) { // if page is NOT marked present
            // TODO
            // map to address 0 with `perm`
            int l1_index = PAGEINDEX(va, 3);
            l1pagetable->entry[l1_index] = 0 | perm;
...
static x86_64_pagetable* lookup_l1pagetable(x86_64_pagetable* pagetable,
    ...
    for (int i = 0; i <= 2; ++i) {
        ...
        if ((perm & PTE_W) && !(pe & PTE_W)) {
            return NULL;
        }
        if ((perm & PTE_U) && !(pe & PTE_U)) {
            return NULL;
        }
