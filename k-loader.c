...
static int program_load_segment(proc* p, const elf_program* ph,
    ...
    for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE) {
        // create new physical page -> assign it to PID -> VMM addr to the physical page
        // x86_64_pagetable* new_p_page = allocator();
        int new_p_page = alloc_free_page(p->p_pid);
        // if (assign_physical_page(new_p_page, p->p_pid) < 0
        if (virtual_memory_map(p->p_pagetable, addr, new_p_page, PAGESIZE,
                                  PTE_P | PTE_W | PTE_U) < 0) {
    ...
    // step 6
    // increment refcount in forking (kernel.c) if page not writtable
    if ((ph->p_flags & ELF_PFLAG_WRITE) == 0) {
    // mark the segment as read only for applications if write flag is not set
    for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE) {
        vamapping vam = virtual_memory_lookup(p->p_pagetable, addr);
        if (vam.pn == -1) {
            return -1;
        }
        // remap the page as read only (PTE_P | PTE_U)
        if (virtual_memory_map(p->p_pagetable, addr, vam.pa, PAGESIZE, PTE_P | PTE_U) < 0) {
            return -1;
        }
    }
}
